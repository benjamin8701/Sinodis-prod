({
	doInit : function(component, event, helper) {
        let getCFRMMsg = $A.get("{!$Label.c.CN_Account_CFRM_Msg}");
        component.set("v.confirmMsg", getCFRMMsg);
        console.log("v.confirmMsg: " + getCFRMMsg);
        let getEditPermissionAndAcountInfoAction = component.get("c.getAccountInfoAndCheckEditPermisison");
        getEditPermissionAndAcountInfoAction.setParams({
            "recordId" : component.get("v.recordId")
        });
        let getEditPermissionAndAcountInfoPromise = helper.callServerAction(component, getEditPermissionAndAcountInfoAction);
        getEditPermissionAndAcountInfoPromise.then(function(_returnValue){
            ...
            if(errorMsg != null && errorMsg != undefined && errorMsg != {} && errorMsg != "") {
                ...
                let getSendMsg = $A.get("{!$Label.c.CN_Account_Send_To_SAP}");
	...
	confirmAction : function(component, event, helper) {
        ...
        let myAction = component.get("c.sendAccount");
        myAction.setParams({
            "recordId": component.get("v.recordId")
        });
