 ...
 * Modify History : 
 **************************************************************************************************/
trigger Sample_Request_Trigger on CN_Sample_Request__c (before insert, before update, before delete, after update,after insert) {
    if(Untils.isTriggerEnabled() && Constants.SAMPLE_REQUEST_TRIGGER_ON) {
        ...
        if(Context.ApplyTo(Constants.CN_Code) && Constants.CN_SAMPLE_REQUEST_TRIGGER_ON) {
            ...
            if(Constants.CN_SR_GENERATE_EVENT_VISIT_HANDLER_TRIGGER_ON){
                triggersObj.bind(Triggers.Evt.afterInsert, new CN_SR_Generate_Event_Visit());
                triggersObj.bind(Triggers.Evt.afterUpdate, new CN_SR_Generate_Event_Visit());
            } 
